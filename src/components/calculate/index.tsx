...
import CustomInputDateLike from "../../ui/custom-input-manual";
...
const Calculate = ({
  ...
  customInput={<CustomInputDateLike />}
  onChangeRaw={(e) => {
    if (!e) return;
    // optional: sanani validatsiya qilish
    const input = (e.target as HTMLInputElement).value;
    
    const dateRegex = /^\d{2}\.\d{2}\.\d{4}$/;
    if (!dateRegex.test(input)) {
      setStartDate(undefined); // noto‘g‘ri format kiritilsa tozalanadi
    }
  }}
/>
  ...
  onChangeRaw={(e) => {
    ...
    const dateRegex = /^\d{2}\.\d{2}\.\d{4}$/;
    if (!dateRegex.test(input)) {
      setEndDate(undefined); // noto‘g‘ri format kiritilsa tozalanadi
    }
  }}
  placeholderText={t("date")}
  dateFormat="dd.MM.yyyy"
  className="w-full"
  calendarClassName="custom-datepicker"
  customInput={<CustomInputDateLike />} // yoki <CustomInputWithManual />
  minDate={startDate}
/>
